...
public class CivilizationSystem extends BaseSystem {
	...
	private void tick(Grid grid, boolean guiExists) 
	{
		...
		{
			requestTurn = false;
			//TODO: Testing code for presentation
			if (turnsPassed % 25 == 0 && turnsPassed != 0)
			{
				for (int i = 0; i < grid.civs.length; i++)
				{
					for (int j = 0; j < grid.civs.length; j++)
						if (!grid.civs[i].equals(grid.civs[j]) && !grid.civs[i].isWar(grid.civs[j]))
							grid.civs[i].war(grid.civs[j]);
				}
			}
